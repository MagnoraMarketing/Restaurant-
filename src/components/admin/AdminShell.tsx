...
export function AdminShell({ demoMode, storage, children }: { demoMode: boolean; storage: string; children: React.ReactNode }) {
  ...
  const nav = (
    <nav className="grid gap-1" aria-label={t("Admin")}>
      {NAV.map((n) => {
  ...
  const switcher = (
    <label className="block">
      <span className="mb-1.5 block text-[10px] font-bold tracking-wider text-ink-400 uppercase">{t("Restaurant (tenant)")}</span>
    ...
    <div className="min-h-dvh bg-ink-950 lg:grid lg:grid-cols-[260px_1fr]">
        ...
        <div className="mt-auto space-y-3">
          {restaurant && (
            <Link href={`/demo/${restaurant.slug}`} target="_blank" className="flex items-center gap-2 rounded-xl border border-white/10 px-3 py-2.5 text-xs text-ink-300 hover:text-white">
              <Icon name="external" className="h-4 w-4" /> {t("Åbn {name}s hjemmeside", { name: restaurant.name })}
            </Link>
          )}
          <p className="rounded-xl bg-white/5 px-3 py-2 text-[11px] text-ink-400">
            {demoMode ? t("🧪 Demo-mode · intet login") : t("🔒 Beskyttet admin")} · {t("lager")}: {t(storage)}
          </p>
          {!demoMode && (
            <button
              onClick={async () => {
            ...
            >
              <Icon name="logout" className="h-4 w-4" /> {t("Log ud")}
            </button>
          )}
        </div>
      </aside>
      ...
      <div className="min-w-0">
        <header className="sticky top-0 z-30 flex h-14 items-center justify-between gap-3 border-b border-white/8 bg-ink-950/85 px-4 backdrop-blur lg:hidden">
          <Logo href="/" sub={false} />
          <button onClick={() => setOpen((o) => !o)} className="grid h-10 w-10 place-items-center rounded-full border border-white/10" aria-label={t("Menu")}>
        ...
        {demoMode && (
          <div className="border-b border-ember-500/20 bg-ember-500/10 px-4 py-2 text-center text-xs text-ember-300 sm:px-8 print:hidden">
            {t("Offentlig admin-demo – data er fiktive og nulstilles løbende. Lav en ordre via")}{" "}
            <Link href={`/demo/${restaurant?.slug ?? ""}`} className="font-semibold underline">
              {t("restaurantens hjemmeside")}
            </Link>{" "}
            {t("eller AI-receptionisten og se den dukke op her.")}
          </div>
        )}
  ...
  return (
    <div className="mb-8 flex flex-wrap items-end justify-between gap-4">
      <div>
        ...
        {text && <p className="mt-1 text-sm text-ink-400">{t(text)}</p>}
      </div>
