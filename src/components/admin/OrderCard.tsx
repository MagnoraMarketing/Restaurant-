  ...
  return (
    <article className={`card animate-pop p-4 ${order.status === "new" ? "ring-1 ring-ember-500/40" : ""}`}>
      <header className="flex items-start justify-between gap-2">
        <div>
          <p className="text-[10px] font-bold tracking-wider text-ink-400 uppercase">Ordre</p>
          <p className="h-display text-2xl leading-none">#{order.orderNumber}</p>
        </div>
        <div className="text-right">
          <span className={`inline-block rounded-full px-2.5 py-1 text-[11px] font-bold ring-1 ${st.cls}`}>{st.label}</span>
          <p className="mt-1 text-[11px] text-ink-400">
            {SOURCE[order.source].icon} {SOURCE[order.source].label} · {timeAgo(order.createdAt)}
          </p>
        </div>
      </header>
        ...
        {order.items.map((i) => (
          <li key={i.id} className="flex justify-between gap-2">
            <span>
              ...
              {describeModifiers(i.modifiers) && <span className="block text-xs text-ember-300">{describeModifiers(i.modifiers)}</span>}
