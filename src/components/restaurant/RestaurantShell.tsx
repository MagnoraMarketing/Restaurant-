    ...
    <CartProvider restaurant={restaurant} menu={menu}>
      <div style={{ ["--accent" as string]: accent }}>
        <div className="relative z-50 flex items-center justify-center gap-3 bg-gradient-to-r from-ember-600 to-ember-500 px-4 py-2 text-center text-xs font-medium text-white">
          <span>🧪 Demo-restaurant på AIbooking – intet login, ordrer ender i admin-demoen.</span>
          <Link href="/" className="shrink-0 font-bold underline underline-offset-2">
            ← Tilbage til AIbooking
          </Link>
        </div>
          ...
          <div className="container-x flex h-16 items-center justify-between gap-3">
            <Link href={`/demo/${restaurant.slug}`} className="flex min-w-0 items-center gap-2.5">
              ...
              <span className="min-w-0">
                ...
                <span className="block truncate text-[11px] text-ink-400">{restaurant.tagline}</span>
              </span>
            </Link>
            <nav className="hidden items-center gap-1 text-sm md:flex">
              <Link href={`/demo/${restaurant.slug}#menu`} className="rounded-full px-3 py-2 text-ink-300 hover:text-white">Menu</Link>
              {restaurant.booking.enabled && <Link href={`/demo/${restaurant.slug}/book`} className="rounded-full px-3 py-2 text-ink-300 hover:text-white">Book bord</Link>}
              <Link href={`/demo/${restaurant.slug}#info`} className="rounded-full px-3 py-2 text-ink-300 hover:text-white">Find os</Link>
            </nav>
            <CartButton />
          </div>
        </header>
        ...
        <footer id="info" className="mt-20 border-t border-white/8 bg-ink-900/60">
          <div className="container-x grid gap-8 py-12 sm:grid-cols-3">
            <div>
              ...
              <p className="mt-2 text-sm text-ink-400">{restaurant.description}</p>
            </div>
            <div className="text-sm">
              <p className="font-semibold">Find os</p>
              ...
              <p className="mt-2 text-xs text-ink-400">🅿️ {restaurant.parking}</p>
            </div>
            <div className="text-sm">
              <p className="font-semibold">Åbningstider</p>
              <dl className="mt-2 space-y-1 text-ink-300">
                {groupedHours(restaurant).map((h) => (
          ...
          <p className="border-t border-white/5 py-4 text-center text-xs text-ink-400">
            AI-receptionist drevet af <Link href="/" className="font-semibold text-white/70 hover:text-white">AIbooking</Link>
          </p>
        </footer>
        <CartDrawer />
