...
export function ProductModal({ product, onClose }: { product: Product; onClose: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-[60] flex items-end justify-center sm:items-center sm:p-6" role="dialog" aria-modal aria-label={product.name}>
      ...
      <div className="relative flex max-h-[92vh] w-full max-w-lg animate-pop flex-col overflow-hidden rounded-t-[28px] border border-white/10 bg-ink-900 sm:rounded-[28px]">
        <div className="relative">
          <FoodImage src={product.image} alt={product.name} emoji={product.emoji} className="h-52 sm:h-60" />
          <button onClick={onClose} className="absolute top-3 right-3 grid h-10 w-10 place-items-center rounded-full bg-black/60 text-white backdrop-blur" aria-label="Luk">
            <Icon name="close" />
          </button>
        </div>
        <div className="flex-1 overflow-y-auto p-5 sm:p-6">
          ...
          <p className="mt-1.5 text-sm text-ink-300">{product.description}</p>
          {product.allergens.length > 0 && <p className="mt-2 text-xs text-ink-400">Allergener: {product.allergens.join(", ")}</p>}
          ...
          {product.modifierGroups.map((g) => (
            <fieldset key={g.id} className="mt-6">
              <legend className="mb-2 flex w-full items-center justify-between text-sm font-semibold">
                {g.name}
                <span className="text-xs font-normal text-ink-400">{g.type === "single" ? (g.required ? "Vælg 1" : "Valgfri") : g.type === "remove" ? "Fravælg" : "Vælg flere"}</span>
              </legend>
          ...
          <label className="mt-6 block">
            <span className="label">Bemærkning til køkkenet</span>
            <input className="input" value={note} onChange={(e) => setNote(e.target.value)} placeholder="Fx allergi eller ønsker" maxLength={200} />
          </label>
        </div>
        <div className="flex items-center gap-3 border-t border-white/8 p-4">
          <div className="flex items-center rounded-full border border-white/10">
            <button onClick={() => setQty((q) => Math.max(1, q - 1))} className="grid h-11 w-11 place-items-center" aria-label="Færre">
              <Icon name="minus" className="h-4 w-4" />
            </button>
            <span className="w-6 text-center font-semibold tabular-nums">{qty}</span>
            <button onClick={() => setQty((q) => Math.min(99, q + 1))} className="grid h-11 w-11 place-items-center" aria-label="Flere">
          ...
          >
            Læg i kurv · {kr(preview.lineTotal)}
          </button>
        </div>
      </div>
    </div>
