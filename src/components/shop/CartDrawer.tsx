...
export function CartButton() {
  ...
  return (
    <button onClick={() => setDrawerOpen(true)} className="btn relative text-white" style={{ background: restaurant.accentColor }} aria-label={`Kurv med ${count} varer`}>
      ...
      <span className="hidden sm:inline">{count > 0 ? kr(subtotal) : t("Kurv")}</span>
  ...
  return (
    <ul className="divide-y divide-white/8">
      {items.map((i) => {
        ...
        const mods = describeModifiers(i.modifiers, product, t);
        return (
          <li key={i.key} className="flex gap-3 py-3.5">
            <div className="min-w-0 flex-1">
              <p className="font-medium">{t(i.name)}</p>
              {mods && <p className="text-xs text-ink-400">{mods}</p>}
              {i.note && <p className="text-xs text-ink-400 italic">“{i.note}”</p>}
              <p className="mt-1 text-xs text-ink-400">{t("{price} pr. stk.", { price: kr(i.unitPrice) })}</p>
            </div>
            <div className="flex flex-col items-end gap-2">
              ...
              {editable ? (
                <div className="flex items-center rounded-full border border-white/10 text-sm">
                  <button onClick={() => setQty(i.key, i.quantity - 1)} className="grid h-8 w-8 place-items-center" aria-label={t("Færre")}>
                    <Icon name="minus" className="h-3.5 w-3.5" />
                  </button>
                  <span className="w-5 text-center tabular-nums">{i.quantity}</span>
                  <button onClick={() => setQty(i.key, i.quantity + 1)} className="grid h-8 w-8 place-items-center" aria-label={t("Flere")}>
                    <Icon name="plus" className="h-3.5 w-3.5" />
                  </button>
                </div>
              ) : (
                <span className="text-xs text-ink-400">{t("Antal: {n}", { n: i.quantity })}</span>
              )}
            </div>
          </li>
        );
...
export function CartDrawer() {
  ...
  return (
    <div className="fixed inset-0 z-[55]" role="dialog" aria-modal aria-label={t("Kurv")}>
      ...
      <aside className="absolute top-0 right-0 flex h-full w-full max-w-md animate-pop flex-col border-l border-white/10 bg-ink-900">
        <div className="flex items-center justify-between border-b border-white/8 px-5 py-4">
          <h2 className="h-display text-xl">{t("Din kurv")}</h2>
          <button onClick={() => setDrawerOpen(false)} className="grid h-10 w-10 place-items-center rounded-full hover:bg-white/5" aria-label={t("Luk")}>
            <Icon name="close" />
          </button>
        </div>
          ...
          {items.length === 0 ? (
            <div className="grid h-full place-items-center text-center text-ink-400">
              <div>
                ...
                <p className="mt-3">{t("Kurven er tom")}</p>
              </div>
            </div>
          ) : (
            <CartLines />
