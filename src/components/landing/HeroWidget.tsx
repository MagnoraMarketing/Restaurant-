    ...
    return (
      <div className="card grid h-[520px] place-items-center p-8 text-center">
        <div>
          ...
          <p className="h-display mt-4 text-2xl">{t("Hej 👋 Jeg er AI-receptionisten")}</p>
          <p className="mt-2 text-ink-300">{t("Jeg kan hjælpe dig med bordreservation, madbestilling eller spørgsmål om restauranten.")}</p>
          <button onClick={() => openReceptionist()} className="btn-primary mt-6">
            <Icon name="mic" className="h-4 w-4" /> {t("Start samtale")}
          </button>
        </div>
      </div>
    );
