  ...
  return (
    <div className="grid gap-5 lg:grid-cols-[1.1fr_1fr]">
      ...
      <div className="relative flex flex-col overflow-hidden rounded-[32px] bg-gradient-to-br from-ember-500 via-ember-600 to-ember-700 p-8 shadow-2xl shadow-ember-600/30">
        ...
        <span className="w-fit rounded-full bg-white px-3 py-1 text-[11px] font-bold text-ink-950">Start her</span>
        <p className="mt-5 text-4xl">{base.emoji}</p>
        <h3 className="h-display mt-3 text-3xl">{base.name}</h3>
        <p className="mt-2 max-w-md text-white/85">{base.text}</p>
        ...
        <ul className="mt-6 grid gap-2 text-sm sm:grid-cols-2">
          {["Mobilvenligt design", "Online menukort", "Åbningstider & kort", "Jeres eget domæne", "Klar til bestilling & booking", "Klar til AI-receptionist"].map((x) => (
            <li key={x} className="flex gap-2"><Icon name="check" className="mt-0.5 h-4 w-4 shrink-0" /> {x}</li>
          ))}
        </ul>
      ...
      <div className="grid gap-5">
        <PriceGroup title="Tilkøb" items={PRICES.addons} />
        <PriceGroup title="AI-receptionist" items={PRICES.ai} highlight />
      </div>
    </div>
  );
}
        ...
        {items.map((i) => (
          <li key={i.key} className="flex items-start gap-4 py-4 first:pt-0 last:pb-0">
            ...
            <p className="shrink-0 text-right">
              ...
              {i.unit && <span className="block text-xs text-ink-400">pr. {i.unit}</span>}
            </p>
          </li>
        ))}
      </ul>
