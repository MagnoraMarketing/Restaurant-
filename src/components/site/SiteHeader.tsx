...
export function SiteHeader() {
  ...
  const trigger = (key: Exclude<MenuKey, null>, label: string) => (
    <button
      ...
      className={`flex items-center gap-1 whitespace-nowrap rounded-full px-2.5 py-2 text-sm transition ${menu === key ? "bg-white/8 text-white" : "text-ink-300 hover:text-white"}`}
    ...
    <header ref={ref} className={`fixed inset-x-0 top-0 z-40 transition ${scrolled || mobile || menu ? "border-b border-white/8 bg-ink-950/90 backdrop-blur-xl" : ""}`} onMouseLeave={() => setMenu(null)}>
      <div className="container-x flex h-16 items-center justify-between gap-4">
        ...
        <nav className="hidden items-center gap-0.5 xl:flex" aria-label="Hovedmenu">
          ...
          <Link href="/#samarbejde" className="whitespace-nowrap rounded-full px-2.5 py-2 text-sm text-ink-300 hover:text-white">{t("Priser")}</Link>
          <Link href="/demo" className="whitespace-nowrap rounded-full px-2.5 py-2 text-sm text-ink-300 hover:text-white">{t("Demo-restauranter")}</Link>
          <Link href="/admin" className="whitespace-nowrap rounded-full px-2.5 py-2 text-sm text-ink-300 hover:text-white">{t("Backend-demo")}</Link>
        </nav>
        <div className="flex items-center gap-2">
          <LanguageSwitcher className="hidden sm:flex" />
          <Link href="/kontakt" className="btn-ghost hidden whitespace-nowrap !py-2.5 2xl:inline-flex">{t("Book demo")}</Link>
          <Link href="/#demo" className="btn-primary hidden whitespace-nowrap !py-2.5 md:inline-flex xl:max-[1400px]:hidden">{t("Prøv AI-receptionisten")}</Link>
      ...
      {/* Mega-menuer (desktop) */}
      {menu && (
        <div className="absolute inset-x-0 top-16 hidden animate-pop border-b border-white/8 bg-ink-950/95 backdrop-blur-xl xl:block">
            ...
            {menu === "loesninger" ? (
              <div className="grid grid-cols-[1fr_280px] gap-8">
                ...
                <div className="rounded-3xl bg-gradient-to-br from-ember-500/25 to-ink-900 p-6 ring-1 ring-ember-500/20">
                  <p className="text-sm font-semibold">{t("Samarbejdsmuligheder")}</p>
                  <p className="mt-2 text-sm text-ink-300">{t("Vælg en pakke – fra AI på telefonen til komplet hjemmeside med bestilling og QR-menu.")}</p>
                  <Link href="/#samarbejde" onClick={() => setMenu(null)} className="btn-primary mt-5 !py-2.5">{t("Se priser")}</Link>
                </div>
              </div>
            ) : (
              <div>
                <p className="mb-4 text-xs font-bold tracking-wider text-ink-400 uppercase">{t("Hvilken type sted har du?")}</p>
                  ...
                  {VENUE_TYPES.map((v) => (
                    <Link key={v.slug} href={`/brancher/${v.slug}`} onClick={() => setMenu(null)} className="group rounded-2xl border border-white/8 bg-ink-900 p-4 transition hover:-translate-y-0.5 hover:border-ember-500/40">
                      ...
                      <span className="mt-1 flex flex-wrap gap-1 text-[10px] text-ink-400">
                        {v.features.booking && <span>{t("Booking")} ·</span>}
                        {v.features.orders && <span>{t("Ordre")} ·</span>}
                        {v.features.delivery && <span>{t("Levering")}</span>}
                      </span>
                    </Link>
                  ))}
                </div>
      ...
      {/* Mobilmenu */}
      {mobile && (
        <nav className="container-x max-h-[calc(100dvh-4rem)] overflow-y-auto pb-6 xl:hidden" aria-label="Mobilmenu">
          {(
            [
          ...
          <Link href="/#demo" onClick={() => setMobile(false)} className="btn-primary mt-4 w-full">{t("Prøv AI-receptionisten")}</Link>
        </nav>
      )}
    </header>
  );
