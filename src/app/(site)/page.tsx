  ...
  return (
    <>
        ...
        <div className="container-x">
          <div className="mx-auto max-w-4xl animate-fade-up text-center">
            <span className="eyebrow">
              <span className="h-1.5 w-1.5 rounded-full bg-ember-400" /> {t("Din restaurant har åbent – også når personalet har travlt")}
            </span>
            <h1 className="h-display mt-6 text-[2.5rem] leading-[1.05] sm:text-6xl lg:text-7xl">
              {t("Din digitale")} <span className="bg-gradient-to-r from-ember-300 via-ember-400 to-ember-600 bg-clip-text text-transparent italic">{t("receptionist")}</span> {t("til restauranten")}
            </h1>
            <p className="mx-auto mt-6 max-w-2xl text-lg text-ink-300">
              {t("AIbooking besvarer kunder, tager imod bestillinger, booker borde og håndterer henvendelser – direkte på hjemmesiden og via telefon.")}
            </p>
            <div className="mt-8 flex flex-wrap justify-center gap-3">
              <a href="#demo" className="btn-primary !px-6 !py-3.5 text-base">
                {t("Prøv AI-receptionisten")} <Icon name="arrow" className="h-4 w-4" />
              </a>
              <a href="#samarbejde" className="btn-secondary !px-6 !py-3.5 text-base">
                {t("Se samarbejdsmuligheder")}
              </a>
            </div>
          </div>
          ...
          <div id="demo" className="relative mt-14 scroll-mt-24 animate-fade-up [animation-delay:150ms]">
            ...
            <p className="mb-3 flex flex-wrap items-center justify-center gap-2 text-center text-xs text-ink-400">
              <span className="h-2 w-2 animate-pulse rounded-full bg-emerald-400" /> {t("Eksempel: {name}s hjemmeside med AIbooking – prøv receptionisten til højre", { name: restaurant.name })}
            </p>
            ...
            <div className="mt-6 flex flex-wrap justify-center gap-3 text-sm">
              <Link href={`/demo/${slug}`} className="btn-secondary !py-2.5">{t("Åbn hele restaurant-siden")}</Link>
              <Link href="/admin/ordrer" className="btn-ghost !py-2.5">{t("Se ordren lande i backend →")}</Link>
            </div>
          </div>
        </div>
      </section>
      ...
      <section className="container-x py-20 sm:py-28">
        <SectionHead
          eyebrow={t("Sådan virker det")}
          title={t("Én AI – alle kanaler")}
          text={t("Uanset om gæsten ringer, taler i widget'en, scanner QR-koden på bordet eller bruger hjemmesiden, ender alt samme sted: som en ordre, en booking eller et svar – direkte i jeres system.")}
        />
        <div className="mt-14">
          <ChannelFlow />
        </div>
      ...
      <section id="samarbejdsmuligheder" className="border-t border-white/8 bg-gradient-to-b from-ink-900/60 to-ink-950 pt-20 sm:pt-28">
        <SectionHead
          eyebrow={t("Samarbejdsmuligheder")}
          title={t("Det tilbyder vi din restaurant")}
          text={t("Vælg de dele I har brug for – fra AI på telefonen til komplet hjemmeside med bestilling og QR-menu. Alt spiller sammen.")}
        />
      </section>

      <Feature t={t} o={offering("telefon")} index={1} visual={<ConversationDemo />}>
        <div className="mt-6 flex flex-wrap items-center gap-4 rounded-2xl border border-white/10 bg-ink-900 p-4">
          ...
          <div className="flex-1">
            <p className="text-xs text-ink-400">{t("Ring til demoen")}</p>
            <p className="font-display text-xl font-semibold tracking-wide">{demoPhone}</p>
          </div>
          {phoneIsReal ? (
            <a href={telHref(demoPhone)} className="btn-primary !py-2.5">{t("Ring nu")}</a>
          ) : (
            <OpenReceptionistButton voice className="btn-primary !py-2.5">{t("Prøv i browseren")}</OpenReceptionistButton>
          )}
        </div>
      </Feature>
      ...
      <Feature t={t} o={offering("voice-widget")} index={2} flip visual={<VoiceWidgetVisual />}>
        <div className="mt-6 flex flex-wrap gap-3">
          <OpenReceptionistButton voice className="btn-primary"><Icon name="mic" className="h-4 w-4" /> {t("Prøv voice widget")}</OpenReceptionistButton>
          <Link href="/admin/ai-assistent" className="btn-secondary">{t("Se opsætning")}</Link>
        </div>
      </Feature>

      <Feature t={t} o={offering("bestilling")} index={3} visual={<OrderCardDemo />}>
        <div className="mt-6 flex flex-wrap gap-3">
          <Link href={`/demo/${slug}#menu`} className="btn-primary">{t("Læg en testordre")}</Link>
          <Link href="/admin/ordrer" className="btn-secondary">{t("Se køkken-skærmen")}</Link>
        </div>
      </Feature>

      <Feature t={t} o={offering("takeaway")} index={4} flip visual={<TakeawayVisual />}>
        <div className="mt-6 flex flex-wrap gap-3">
          <Link href={`/demo/${slug}#menu`} className="btn-primary">{t("Bestil takeaway")}</Link>
          <OpenReceptionistButton message={t("Jeg vil gerne bestille takeaway")} className="btn-secondary">{t("Bestil via AI")}</OpenReceptionistButton>
        </div>
      </Feature>

      <Feature t={t} o={offering("bordbooking")} index={5} visual={<BookingVisual />}>
        <div className="mt-6 flex flex-wrap gap-3">
          <Link href={`/demo/${slug}/book`} className="btn-primary">{t("Book et bord")}</Link>
          <OpenReceptionistButton message={t("Book bord til 4 personer fredag kl. 19:00")} className="btn-secondary">{t("Book via AI")}</OpenReceptionistButton>
        </div>
      </Feature>

      <Feature t={t} o={offering("qr-nfc")} index={6} flip visual={<QrNfcVisual slug={slug} />}>
        <div className="mt-6 flex flex-wrap gap-3">
          <Link href={`/m/${slug}?bord=7`} className="btn-primary">{t("Åbn menukortet som bord 7")}</Link>
          <Link href="/admin/qr-nfc" className="btn-secondary">{t("QR & NFC i backend")}</Link>
        </div>
      </Feature>

      {/* ============================================ QR & NFC – sådan opretter du */}
      <section className="container-x pb-16 sm:pb-24">
        <div className="mb-8 flex flex-wrap items-end justify-between gap-4">
          <div className="max-w-2xl">
            <span className="eyebrow">{t("QR & NFC i dit login")}</span>
            <h2 className="h-display mt-4 text-3xl sm:text-4xl">{t("Sådan opretter du QR-koder og NFC-chips")}</h2>
            <p className="mt-3 text-ink-300">{t("Vælg hvad koden skal åbne, hvor mange borde du har – og print. NFC-chippen til anmeldelser styrer du selv: skift linket når som helst.")}</p>
          </div>
          <p className="rounded-full border border-ember-500/30 bg-ember-500/10 px-4 py-2 text-sm font-semibold text-ember-300">{t("30 € pr. stk.")}</p>
        </div>
        <QrNfcSetupVisual slug={slug} name={restaurant.name} />
      </section>

      <Feature t={t} o={offering("hjemmeside")} index={7} visual={<WebsitesVisual restaurants={restaurants} />}>
        <div className="mt-6 flex flex-wrap gap-3">
          <Link href="/demo" className="btn-primary">{t("Se eksempler")}</Link>
          <Link href="/kontakt" className="btn-secondary">{t("Få en hjemmeside")}</Link>
        </div>
      </Feature>
      ...
      <section className="container-x py-20 sm:py-28">
        <div className="grid items-center gap-12 lg:grid-cols-[0.8fr_1.2fr]">
          <div>
            ...
            <h2 className="h-display mt-5 text-4xl sm:text-5xl">{t("Alt samlet i ét visuelt overblik")}</h2>
            <p className="mt-4 text-lg text-ink-300">{t("Se opkald live, lyt til hvad AI'en har aftalt, og styr ordrer, reservationer, menu, QR-koder og integrationer – pr. restaurant.")}</p>
            <ul className="mt-6 grid gap-2 text-sm text-ink-300">
              {["📞 Opkaldslog med udskrift og resultat", "🧾 Live køkken-skærm: Accepter / Afvis / Klar", "📅 Reservationer og selskaber", "📱 QR-koder og NFC-links pr. bord", "✨ AI-receptionistens viden og regler"].map((x) => (
                <li key={x} className="rounded-2xl border border-white/8 bg-ink-900 px-4 py-3">{t(x)}</li>
              ))}
            </ul>
            <div className="mt-8 flex flex-wrap gap-3">
              <Link href="/admin" className="btn-primary">{t("Åbn backend-demoen")}</Link>
              <Link href="/admin/opkald" className="btn-secondary">{t("Se opkald")}</Link>
            </div>
          </div>
          <BackendPreview />
        </div>
      ...
      <section id="samarbejde" className="scroll-mt-20 border-y border-white/8 bg-ink-900/40 py-20 sm:py-28">
        <div className="container-x">
          <SectionHead eyebrow={t("Priser & samarbejde")} title={t("Start med en hjemmeside – byg på efter behov")} text={t("Vælg en standard hjemmeside med menukort, og tilføj QR-kode, NFC-anmeldelser og AI-receptionist, når I er klar.")} />
          <div className="mt-12">
            <Pricing />
          </div>
          <p className="mt-8 text-center text-sm text-ink-400">
            {t("Spørgsmål til priser eller en samlet løsning?")} <Link href="/kontakt" className="font-semibold text-ember-300 hover:text-ember-400">{t("Book en uforpligtende demo →")}</Link>
          </p>
        </div>
      </section>
      ...
      <section id="brancher" className="container-x scroll-mt-20 py-20 sm:py-28">
        <SectionHead eyebrow={t("Vælg type")} title={t("Én løsning – mange typer restauranter")} text={t("Se hvordan AI-receptionisten hjælper netop jeres type sted – med bordreservation, bestilling, takeaway og levering.")} />
        <div className="mt-12 grid grid-cols-2 gap-4 md:grid-cols-3 lg:grid-cols-5">
          {VENUE_TYPES.map((v) => (
            <Link key={v.slug} href={`/brancher/${v.slug}`} className="group relative overflow-hidden rounded-3xl border border-white/8">
              <FoodImage src={v.image} alt={t(v.name)} emoji={v.emoji} className="h-44 transition duration-500 group-hover:scale-105 sm:h-52" />
              <div className="absolute inset-0 bg-gradient-to-t from-black/90 via-black/30 to-transparent" />
              <div className="absolute inset-x-0 bottom-0 p-4">
                <p className="font-semibold">{v.emoji} {t(v.name)}</p>
                <p className="mt-1 flex flex-wrap gap-1 text-[10px] text-white/70">
                  {v.features.booking && <span className="rounded-full bg-white/15 px-1.5 py-0.5">{t("Booking")}</span>}
                  {v.features.orders && <span className="rounded-full bg-white/15 px-1.5 py-0.5">{t("Ordre")}</span>}
                  {v.features.delivery && <span className="rounded-full bg-white/15 px-1.5 py-0.5">{t("Levering")}</span>}
                  {v.features.qr && <span className="rounded-full bg-white/15 px-1.5 py-0.5">{t("QR")}</span>}
                </p>
              </div>
            </Link>
          ))}
      ...
      <section className="border-y border-white/8 bg-ink-900/40 py-20 sm:py-24">
        <div className="container-x grid items-center gap-10 lg:grid-cols-2">
          <SectionHead align="left" eyebrow={t("Restauranten i centrum")} title={t("Din restaurant. Dit brand. Din ordre.")} text={t("AIbooking tvinger ikke restauranten ind i en bestemt bestillingsoplevelse. AIbooking ligger i baggrunden som den intelligente receptionist – AIbooking leverer teknologien.")} />
          <ul className="grid grid-cols-1 gap-3 sm:grid-cols-2">
            {["Sin egen hjemmeside", "Sit eget design", "Sin egen menu", "Sine egne priser", "Sit eget ordersystem", "Sin egen booking", "Sit eget telefonnummer", "Sine egne integrationer"].map((x) => (
              <li key={x} className="flex items-center gap-3 rounded-2xl border border-white/8 bg-ink-900 px-4 py-3 text-sm">
                <Icon name="check" className="h-4 w-4 text-ember-400" /> {t(x)}
              </li>
            ))}
          </ul>
        </div>
      ...
      <section id="integrationer" className="container-x scroll-mt-20 py-20 sm:py-28">
        <SectionHead eyebrow={t("Integrationer")} title={t("Forbind med det system du allerede bruger")} text={t("Restauranten behøver ikke skifte hele sit system for at bruge AIbooking.")} />
        <div className="mt-12 grid gap-4 sm:grid-cols-2 lg:grid-cols-5">
          {INTEGRATIONS.map((i) => (
            <div key={i.title} className="card relative p-6">
              {i.tag && <span className="absolute top-4 right-4 rounded-full bg-ember-500/15 px-2 py-0.5 text-[10px] font-bold text-ember-300 uppercase">{t(i.tag)}</span>}
        ...
        <div id="api" className="mt-10 grid scroll-mt-24 gap-6 rounded-[28px] border border-white/8 bg-ink-900/60 p-6 sm:p-8 lg:grid-cols-2">
          <div>
            <p className="text-sm font-semibold text-ember-300">{t("API-first")}</p>
            <p className="h-display mt-2 text-2xl">{t("Ordrer fra alle kanaler via ét REST API")}</p>
            <p className="mt-2 text-sm text-ink-400">{t("Hjemmeside, AI Voice, chat-widget, telefon, QR/NFC, Shopify, POS-systemer og andre ordersystemer – med signerede webhooks.")}</p>
          </div>
      ...
      <section className="bg-gradient-to-b from-ink-950 via-ink-900/60 to-ink-950 py-20 sm:py-28">
        <div className="container-x">
          <SectionHead eyebrow={t("Fordele")} title={t("Mindre tid på telefonen. Mere tid til gæsterne.")} />
          <div className="mt-12 grid gap-4 sm:grid-cols-2 lg:grid-cols-5">
            {BENEFITS.map((b) => (
      ...
      <section className="container-x pb-24">
        <div className="relative overflow-hidden rounded-[40px] bg-gradient-to-br from-ember-500 via-ember-600 to-ember-700 p-10 text-center sm:p-16">
          ...
          <h2 className="h-display relative mx-auto max-w-3xl text-4xl sm:text-5xl">{t("Se hvad AIbooking kan gøre for din restaurant")}</h2>
          <p className="relative mx-auto mt-4 max-w-xl text-lg text-white/85">{t("Prøv AI-receptionisten direkte på hjemmesiden eller book en demonstration.")}</p>
          <div className="relative mt-8 flex flex-col justify-center gap-3 sm:flex-row">
            <a href="#demo" className="btn bg-white !px-7 !py-3.5 text-base text-ink-950 hover:bg-cream">{t("Prøv gratis")}</a>
            <Link href="/kontakt" className="btn border border-white/40 !px-7 !py-3.5 text-base text-white hover:bg-white/10">{t("Book demo")}</Link>
          </div>
        </div>
      </section>
          ...
          <ul className="mt-6 grid gap-2 sm:grid-cols-2">
            {o.bullets.map((b) => (
              <li key={b} className="flex gap-2 text-sm text-ink-300">
                <Icon name="check" className="mt-0.5 h-4 w-4 shrink-0 text-ember-400" /> {t(b)}
              </li>
            ))}
          </ul>
          {children}
