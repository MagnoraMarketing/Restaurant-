  ...
  return (
    <>
      ...
      <section className="container-x -mt-2 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
        <InfoCard icon="pin" title={t("Adresse")}>
          {r.address}, {r.city}
        </InfoCard>
        <InfoCard icon="clock" title={t("Åbningstider")}>
          {groupedHours(r, locale).map((h) => (
            <span key={h.label} className="block">
              {h.label}: {h.value}
            </span>
          ))}
        </InfoCard>
        <InfoCard icon="truck" title={r.delivery.enabled ? t("Levering") : t("Afhentning")}>
          {r.delivery.enabled
            ? t("{fee} · ca. {min} min · min. {minimum}", { fee: kr(r.delivery.fee), min: r.delivery.estimatedMinutes, minimum: kr(r.delivery.minimumOrder) })
            : t("Klar på ca. {min} min.", { min: r.pickup.estimatedMinutes })}
        </InfoCard>
        <div className="card flex flex-col justify-between gap-3 p-5" style={{ background: `linear-gradient(150deg, color-mix(in oklab, ${accent} 28%, #121214), #121214)` }}>
          <p className="text-sm">
            <span className="font-semibold">{t("Spørg vores AI-receptionist")}</span>
            <span className="block text-white/70">{t("Bestil, book bord eller spørg om allergener.")}</span>
          </p>
          <div className="flex gap-2">
            ...
            <OpenReceptionistButton voice className="btn-secondary flex-1 !py-2 text-xs">
              <Icon name="mic" className="h-4 w-4" /> {t("Tal")}
            </OpenReceptionistButton>
          </div>
        </div>
      </section>
      ...
      <section id="menu" className="container-x scroll-mt-20 pt-16">
        <div className="mb-6 flex flex-wrap items-end justify-between gap-4">
          <div>
            ...
            <h2 className="h-display mt-1 text-4xl">{t("Bestil mad")}</h2>
          </div>
          <p className="max-w-sm text-sm text-ink-400">{t("Vælg produkter og tilvalg, læg i kurv og vælg levering eller afhentning. Eller skriv/sig det bare til AI-receptionisten.")}</p>
        </div>
        <MenuSection />
      </section>
    </>
