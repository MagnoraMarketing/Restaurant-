...
export async function generateMetadata(): Promise<Metadata> {
  const { t } = await getT();
  return { title: t("Demo-restauranter") };
}
  ...
  return (
    <>
      ...
      <main className="container-x pt-32 pb-24">
        ...
        <h1 className="h-display mt-5 text-4xl sm:text-6xl">{t("Demo-restauranter")}</h1>
        <p className="mt-4 max-w-2xl text-lg text-ink-300">
          {t("Hver restaurant er en selvstændig tenant med sin egen hjemmeside, sit eget design, sin egen menu, sit eget telefonnummer og sin egen AI-agent – på samme platform, uden at data blandes.")}
        </p>
        <div className="mt-12 grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
          {restaurants.map((r) => (
            <Link key={r.id} href={`/demo/${r.slug}`} className="card group overflow-hidden transition hover:-translate-y-1 hover:border-white/20">
              ...
              <div className="p-6">
                <span className="rounded-full px-2.5 py-1 text-[11px] font-bold text-white" style={{ background: r.accentColor }}>{t(LABEL[r.industry])}</span>
                <h2 className="mt-3 font-display text-2xl font-semibold">{r.emoji} {r.name}</h2>
                <p className="text-sm text-ink-400">{t(r.tagline)}</p>
                <p className="mt-3 text-xs text-ink-400">
                  {[r.delivery.enabled && t("Levering"), r.pickup.enabled && t("Afhentning"), r.booking.enabled && t("Bordbooking")].filter(Boolean).join(" · ")}
                </p>
              </div>
            </Link>
          ))}
